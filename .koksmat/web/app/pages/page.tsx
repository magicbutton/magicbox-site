...
import { KoksmatContext } from "../koksmat/context";
...
function translateImageUrl(url: string) {
  return url
    .replace("https://{fqdn}{site}/", "https://365admin.blob.core.windows.net/sites/magic365/home/")
    .replace("{site}/", "https://365admin.blob.core.windows.net/sites/magic365/home/")

}

function Hero(props: { webpartData: any }) {
  ...
  // return <div>Hero</div>
  return <div>
    {koksmat.options.showDebug &&
      <div>
        <div>Hero</div>
        {/* <textarea value={JSON.stringify(data, null, 2)}> </textarea>
        {data.properties.content.map((content, key) => {
          return <pre key={key}>
    ...
    {data.properties.content.filter((content,ix)=>ix<numberOfItems).map((content, key) => {
      ...
      let cta = ""
      let isEven = key % 2 === 0
      const searchablePlainTexts = data.serverProcessedContent.searchablePlainTexts as any
      switch (key) {
        case 0:
      ...
      let render = null
      if (factor === 1) {
        render = <div className="relative" ><img src={img} alt={content.alternateText} />
          {content.showCallToAction && <div className="absolute left-10 top-2/3 z-10 text-2xl font-bold text-white">
            {title}
          </div>}
          {content.showCallToAction && <div className="absolute bottom-10 left-10 z-10 text-lg text-white">
            <Link href="/profile" className="cursor-pointer"> {cta}</Link>
          </div>}
        </div>
      } else {
        render = <div className={!isEven ? "flex flex-row-reverse" : " flex  flex-row"}  >
          <div className={cn("w-2/3 ")} style={{background:"url('"+img+"')",backgroundPosition:"center",backgroundSize:"cover"}}>
            
          </div>
          <div className="w-1/3 p-10">
            {content.showFeatureText && <span className="text bg-[#3F7288] px-3 py-1 text-sm uppercase  text-white">
              {content.featureText}
            </span>}
            ...
            {content.showDescription && <div className="text py-3">
              
              {content.description}
            </div>}

            {content.showCallToAction && <div className=" bottom-10 left-10 z-10 py-3 text-lg">
              {cta}
            </div>}
          </div>
        </div>
...
function Text(props: { webpartData: any }) {
  const { webpartData } = props
  const koksmat = useContext(KoksmatContext)
  const data: Text = webpartData

  const html = data["pnp:CanvasControlProperties"]["pnp:CanvasControlProperty"]["@_Value"]
  return <div>
    {/* <div>Text</div> */}
    <div>
...
function Webpart(props: { webpartData: any }) {
  ...
  switch (webpartType) {
    case "Hero":
      return <Hero webpartData={props.webpartData} />
      break;
    case "Text":
      return <Text webpartData={props.webpartData} />
      break;
    default:
      break;
  }
...
function CanvasControl(props: { webpartData: any }) {
  const { webpartData } = props
  const controls = Array.isArray(webpartData["pnp:CanvasControl"]) ? webpartData["pnp:CanvasControl"] : [webpartData["pnp:CanvasControl"]]
...
export default function HomePage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const pages: PnpClientSidePage[] = [template["pnp:Provisioning"]["pnp:Templates"]["pnp:ProvisioningTemplate"]["pnp:ClientSidePages"]["pnp:ClientSidePage"]]
      setpages(pages);
    ...
    <div className="container">
      ...
      {pages && pages.map((page, key) => {
        return <div key={key}>
          ...
          {page["pnp:Sections"]["pnp:Section"].sort((a, b) => {
            const a1 = parseInt(a["@_Order"])
            const b1 = parseInt(b["@_Order"])
            return a1 - b1

          }).map((section, key) => {
            return <div key={key} className="pb-10">
              {/* Section: */}
